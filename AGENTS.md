...
2. **主键 UUIDv7 标准与字段精简**：
   - 数据库实体主键统一采用 **UUIDv7**（基于时间戳排序的 UUID），完美兼容 PostgreSQL 17/18 及以上版本（在 PG18 原生 uuidv7 或各版本中均具有极致的 B-Tree 索引聚集与写入性能）。
   - 去除无实际业务意义的冗余 `title` 字段，精简数据模型。
3. **保留 ORB 核心高精度匹配，构建多算法扩展体系**：
...
uv run python reset_db.py                        # 重置数据库结构
```
...
- **错误处理**: 统一抛出 `HTTPException` 或标准 JSON 格式响应。
...
1. **统一数据库模型与迁移** (`app/domain/models/secret_entry.py` / `app/services/database.py`):
   - `SecretEntry` 表字段包含 `id (UUIDv7)`, `credential_type`, `credential_value (db_index=True)`, `algorithm`, `feature_data`, `keypoints_count`, `secret_text`, `extra_features`, `created_at`, `viewed_at`。
   - `credential_value` 为普通非唯一索引，支持一对多存储。
...
3. **决策匹配引擎** (`app/services/match_engine.py`):
...
- **主键生成**: `uuid6` (生成标准 UUIDv7，兼容 PG17 / PG18 原生 uuidv7)
- **图像处理与算法库**: 
